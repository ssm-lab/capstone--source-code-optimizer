...
class OutputManager:
    def __init__(self, base_dir: Path | None = None):
        """
        Initializes and manages log files.

        Args:
            base_dir (Path | None): Base directory for storing logs. Defaults to the user's home directory.
        """
        if base_dir is None:
            base_dir = Path.home()

        self.base_output_dir = Path(base_dir) / ".ecooptimizer"
        self.output_dir = self.base_output_dir / "outputs"
        self.logs_dir = self.output_dir / "logs"

        self._initialize_output_structure()
        self.log_files = {
            "main": self.logs_dir / "main.log",
            "detect_smells": self.logs_dir / "detect_smells.log",
            "refactor_smell": self.logs_dir / "refactor_smell.log",
        }
        self._setup_loggers()
    ...
    def _setup_loggers(self):
        ...
        self.loggers = {
            "detect_smells": self._create_logger(
                "detect_smells", self.log_files["detect_smells"], self.log_files["main"]
            ),
            "refactor_smell": self._create_logger(
                "refactor_smell", self.log_files["refactor_smell"], self.log_files["main"]
            ),
        }
    ...
    def _create_logger(self, name: str, log_file: Path, main_log_file: Path):
        ...
        formatter = logging.Formatter(
            "[ecooptimizer %(levelname)s @ %(asctime)s] %(message)s", "%H:%M:%S"
        )
    ...
    def save_json_files(self, file_name: str, data: dict[Any, Any] | list[Any]):
    ...
    def copy_file_to_output(self, source_file_path: Path, new_file_name: str):
        ...
        destination_path = self.output_dir / new_file_name
