...
def detect_repeated_calls(file_path: Path, tree: ast.AST, threshold: int = 2):
    ...
    def analyze_node(node: ast.AST):
        """
        Analyzes a given node for repeated function calls.

        Parameters:
            node (ast.AST): The node to analyze.
        """
        call_counts = defaultdict(list)  # Tracks occurrences of each call
        modified_lines = set()  # Tracks lines with variable modifications
        ...
        # Process detected repeated calls
        for call_string, occurrences in call_counts.items():
            if len(occurrences) >= threshold:
                # Skip if repeated calls are interrupted by modifications
