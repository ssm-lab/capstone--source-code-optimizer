...
\begin{tabularx}{\textwidth}{p{3.7cm}>{\raggedright\arraybackslash}p{1.8cm}X}
  ...
  March 24th, 2025 & Sevhena Walker & Updated solution constraints, style requirements, wider environment reqs. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|c|
    ...
    5 & Configure Smell Display & User & Display Preferences (in), Updated VS Code Settings (out) \\
    6 & Select Code Smell & User & Selected Code Smell (in) \\
    7 & Refactor Code & VS Code Plugin (via Refactoring Lib.) & Code Segment (in), Refactored Code (out) \\
    8 & Measure Energy (Before) & Refactoring Library (via Energy Tool) & Original Code (in), Initial Energy Results (out) \\
    9 & Measure Energy (After) & Refactoring Library (via Energy Tool) & Refactored Code (in), Final Energy Results (out) \\
    10 & Show Results & VS Code Plugin & Refactored Code, Performance Metrics (out) \\
    11 & Accept/Reject Results & User & Refactored Code, Energy Results (in), Decision (out) \\
    12 & Clear Smell History & User & Clear Command (in), Reset Cache (out) \\
    13 & Access Documentation & User & Help Request (in), Documentation (out) \\
    14 & Generate System Logs & System & Log Request (in), Log File (out) \\
    \bottomrule
  \end{tabularx}
