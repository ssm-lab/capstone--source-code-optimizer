...
\begin{enumerate}[label=FR \arabic*., wide=0pt, leftmargin=*]
    ...
    {\bf Fit Criterion:} The refactoring process must be triggered automatically on GitHub commits, with the number of refactorings improving energy efficiency without introducing any functional errors meeting the REFACTOR\_EFFICACY\_THRESH.\\
    {\bf Priority:} Medium
...
\begin{enumerate}[label=LFR-AP \arabic*., wide=0pt, leftmargin=*]
    ...
    {\bf Fit Criterion:} The GitHub Action must display a success icon or green label in the PR summary if energy savings exceed the CRITICAL\_ENERGY\_SAVE\_THRESH.\\
    {\bf Priority:} Low
\end{enumerate}
