...
\begin{enumerate}
    \item \textbf{Requirement:} The refactoring interface should clearly present the original and refactored code side by side, allowing developers to easily compare and choose between them.
    \begin{itemize}[label={}]
    ...
    \end{itemize}
    \item \textbf{Requirement:} The plugin should have a minimalist design, focusing only on essential elements to reduce clutter.
    \begin{itemize}[label={}]
        \item \textbf{Fit Criteria:} The interface should prominently display only the code, refactoring suggestions, and energy metrics, without unnecessary visual elements or distractions.
    \end{itemize}
    \item \textbf{Requirement:} The plugin should adapt to the user’s VS Code theme, supporting both light and dark modes.
    \begin{itemize}[label={}]
        \item \textbf{Fit Criteria:} The plugin’s interface must automatically adjust to match the user’s current VS Code theme settings (light or dark), with no manual changes required.
    \end{itemize}
    ...
    \begin{itemize}[label={}]
        \item \textbf{Fit Criteria:} The plugin must use colour-coded indicators (e.g., yellow for minor energy savings, red for major savings), and developers should be able to identify refactoring opportunities quickly.
    \end{itemize}
...
\begin{enumerate}
    \item \textbf{Requirement:} The  IDE plugin and GitHub action tool should convey a professional and authoritative appearance to instill confidence in developers.
    \begin{itemize}[label={}]
        \item \textbf{Fit Criteria:} After their first encounter with the tools, at least 60\% of representative developers should feel that it is a trustworthy and reliable solution for energy-efficient refactoring.
    \end{itemize}
    ...
    \end{itemize}
    \item \textbf{Requirement:} The IDE plugin and GitHub action tools' design should be visually appealing and modern, aligning with contemporary software development tools.
    \begin{itemize}[label={}]
        \item \textbf{Fit Criteria:} At least 75\% of users should express satisfaction with the tools' visual design and layout after their initial interaction.
    \end{itemize}
\end{enumerate}
