...
global coverage thresholds of 80\% for the following reasons.

\noindent \textbf{Extension Core Logic:} The
file \texttt{extension.ts}, which contains the core logic for the
VSCode extension, has 0\% coverage as it is mainly made up of
initialization commands with no real logic that can be tested.

\noindent \textbf{UI Components:} The
...
\noindent \textbf{Partial Coverage:} The file \texttt{refactorSmell.ts} has moderate coverage (55.37\%
...
\noindent \textbf{Future Testing:} \texttt{configManager.ts} has not been tested as yet
