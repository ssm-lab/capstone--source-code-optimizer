...
\section{MIS of Extension Initiator}

\subsection{Module}
\texttt{Extension Initiator} is a module that initializes the extension and registers commands for VS Code Plugin.
...
\subsubsection{Assumptions}
\begin{itemize}
\item The extension is correctly loaded in VS Code.
\item Ecooptimizer executable is reachable and operational.
\end{itemize}

\subsubsection{Access Routine Semantics}
\texttt{activate()}
\begin{itemize}
\item \textbf{Transition}: Activates the extension and registers commands.
\item \textbf{Output}: None.
\item \textbf{Exception}: None.
\end{itemize}
...
\subsection{Module}
\texttt{BackendCommunicator} handles all communication between the extension and the backend service. It sends requests for analysis or refactoring and receives results.

\subsection{Uses}
Ecooptimizer executable for energy measurement, smell detection and refactoring of applications.
...
\subsubsection{Environment Variables}
\begin{itemize}
\item \textbf{NETWORK}: Used for communicating with Ecooptimizer.
...
\subsubsection{Access Routine Semantics}
\texttt{sendRequest(requestType: string, data: any)}
\begin{itemize}
\item \textbf{Transition}: Sends the provided request to Ecooptimizer and receives a response.
\item \textbf{Output}: A promise that resolves with the Ecooptimizer's response.
...
\subsection{Module}
\texttt{Smell Detector} analyzes the active file for code smells and interacts with Ecootimizer for detection.

\subsection{Uses}
\begin{itemize}
\item \texttt{Backend Communicator} for communicating with  Ecootimizer for smell detection.
...
\item Ecooptimizer correctly identifies smells.
\end{itemize}

\subsubsection{Access Routine Semantics}
\texttt{detect()}
\begin{itemize}
\item \textbf{Transition}: Reads the active file, sends it to Ecooptimizer for analysis, and highlights detected smells in the editor.
\item \textbf{Output}: None.
...
\subsection{Uses}
\begin{itemize}
\item \texttt{Backend Communicator} for sending the smell data to Ecooptimizer for refactoring.
